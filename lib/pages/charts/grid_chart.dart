...
class GridChart extends StatelessWidget {
  ...
  ChartData _gridData(BuildContext context) =>
      context.read<ThingControllerCubit>().state.charts!.gridData!;
      ...
      Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [
        Column(
          children: [
            ...
            Text("${_gridData(context).minV}"),
          ],
        ),
        Column(
          children: [
            Text(S.of(context)!.maximum),
            Text("${_gridData(context).maxV}"),
          ],
        )
      ]),
      AspectRatio(
  ...
  List<FlSpot> _getFlSpotData(BuildContext context) {
    return List.generate(_gridData(context).data.length, (index) {
      return FlSpot(index.toDouble(),
          _gridData(context).data.values.toList()[index].toDouble());
    });
  }
  ...
  Widget getTitles(BuildContext context, double value, TitleMeta meta) {
    return Text(
        DateFormat('HH:mm').format(DateTime.fromMillisecondsSinceEpoch(
            _gridData(context).data.keys.toList()[value.toInt()] * 1000)),
    ...
    return touchedSpots.map((LineBarSpot touchedSpot) {
      ...
      String time = DateFormat('HH:mm').format(
          DateTime.fromMillisecondsSinceEpoch(
              _gridData(context).data.keys.toList()[touchedSpot.x.toInt()] *
                  1000));
